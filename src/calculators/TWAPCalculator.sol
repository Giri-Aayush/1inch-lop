...
contract TWAPCalculator is IAmountCalculator {
    ...
    function _validateExecutionTiming(TWAPData memory twapData) internal view {
        ...
        if (twapData.executedIntervals > 0 && twapData.intervals > 0) {
            ...
            uint256 expectedExecutionTime = twapData.startTime + 
                                          (twapData.executedIntervals * intervalDuration);
    ...
    function _applyVolatilityAdjustment(
        ...
        if (twapData.maxSlippageBPS > 500) { // > 5% slippage = high volatility
            volatilityFactor = 85; // Reduce amount by 15%
        } else if (twapData.maxSlippageBPS > 200) { // > 2% slippage = medium volatility
        ...
        ) 
    {
        ...
        isActive = currentTime >= twapData.startTime && 
                   currentTime <= twapData.startTime + twapData.duration &&
        ...
        if (isActive && twapData.intervals > 0) {
            ...
            uint256 intervalDuration = twapData.duration / twapData.intervals;
            nextExecutionTime = twapData.startTime + 
                              (twapData.executedIntervals * intervalDuration) + 
                              twapData.minExecutionGap;
        }
    }
}
