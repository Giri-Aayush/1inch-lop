...
abstract contract BaseMainnetTest is Test {
    ...
    function _fundTestAccounts() internal {
        ...
        IERC20(USDC).transfer(alice, 80000e6);
        IERC20(USDC).transfer(bob, 60000e6);
        IERC20(USDC).transfer(carol, 40000e6);
        IERC20(USDC).transfer(dave, 20000e6);
        vm.stopPrank();
    }
    
    // ============ ORDER HELPERS ============
    
    function _createETHSellOrder(uint256 ethAmount, uint256 pricePerETH) 
    ...
    function _createETHBuyOrder(uint256 ethAmount, uint256 pricePerETH) 
        internal view returns (IOrderMixin.Order memory) 
    {
        return IOrderMixin.Order({
    ...
    function _createMultiAssetOrder(address makerAsset, address takerAsset, uint256 makingAmount, uint256 takingAmount)
        internal view returns (IOrderMixin.Order memory)
    {
        return IOrderMixin.Order({
    ...
    function _createExtremeVolatilityData() internal view returns (EnhancedVolatilityCalculator.VolatilityData memory) {
        return EnhancedVolatilityCalculator.VolatilityData({
            baselineVolatility: 300,        // 3%
    ...
    ) internal pure returns (EnhancedTWAPVolatilityExecutor.CombinedStrategyData memory) {
    ...
    function _createCallOptionData(uint256 strikePrice, uint256 expiration, uint256 premium) 
        internal view returns (OptionsCalculator.OptionData memory) 
    {
    ...
    function _createPutOptionData(uint256 strikePrice, uint256 expiration, uint256 premium) 
        internal view returns (OptionsCalculator.OptionData memory) 
    {
        return OptionsCalculator.OptionData({
            strikePrice: strikePrice,
    ...
    function _assertVolatilityBounds(uint256 amount, EnhancedVolatilityCalculator.VolatilityData memory volData) internal {
